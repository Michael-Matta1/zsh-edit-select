...
/* Persistent window reused by daemon-mode selection reads to avoid
   ...
   Set once after daemon() in run_daemon(); None in short-lived modes. */
...
/* Forward declaration — defined further down; needed by check_and_update_primary(),
   check_and_update_clipboard(), and run_oneshot(), which are defined before get_selection(). */
...
/* When true, the daemon also subscribes to CLIPBOARD owner-change events
   via XFixes and writes clipboard content to the same cache files as
   PRIMARY.  Enabled by --monitor-clipboard for WSL2 where Windows Terminal
   does not set X11 PRIMARY on mouse selection; WSLg bridges the Windows
   clipboard to X11 CLIPBOARD, so monitoring CLIPBOARD lets the plugin
   detect mouse selections made with copyOnSelect enabled. */
static bool monitor_clipboard = false;

/* Persistent fds for write_primary() daemon hot path.
   Opened once after daemon() in run_daemon(); reused for all subsequent writes.
   -1 = not yet open (pre-daemon initial write uses the open/write/close
   fallback). */
...
static int ensure_cache_dir(const char *dir) {
  ...
  if (dir && dir[0]) {
    ...
    if (runtime) {
      n = snprintf(cache_dir, sizeof(cache_dir), "%s/zsh-edit-select-%d", runtime,
                   (int)getuid());
    } else if (access("/dev/shm", W_OK | X_OK) == 0) {
      n = snprintf(cache_dir, sizeof(cache_dir), "/dev/shm/zsh-edit-select-%d",
                   (int)getuid());
    } else {
      const char *home = getenv("HOME");
      if (!home)
        return -1;
      n = snprintf(cache_dir, sizeof(cache_dir), "%s/.cache/zsh-edit-select", home);
    }
  }
  ...
  n = snprintf(primary_path, sizeof(primary_path), "%s/%s", cache_dir,
               PRIMARY_FILE);
...
/* Re-read CLIPBOARD and update cache, same as check_and_update_primary()
   but for the CLIPBOARD selection.  Used on WSL2 where Windows Terminal
   mouse selections go through the Windows clipboard → WSLg → X11 CLIPBOARD
   path rather than X11 PRIMARY. */
static void check_and_update_clipboard(void) {
  size_t len = 0;
  char *sel = get_selection(xa_clipboard, &len);
...
/* Read the specified X11 selection (PRIMARY or CLIPBOARD) into a malloc'd
 * buffer. Uses xa_primary or xa_clipboard directly as the property atom for the
...
static char *get_selection(Atom selection, size_t *out_len) {
  ...
  *out_len = 0;
  ...
  if (got_notify && ev.xselection.property != None) {
    ...
    if (XGetWindowProperty(dpy, w, prop_atom, 0, MAX_SELECTION_SIZE / 4, False,
...
static char *read_all_stdin(size_t *out_len) {
  ...
  while (1) {
    if (total + 4096 > capacity) {
      capacity *= 2;
      if (capacity > MAX_SELECTION_SIZE)
        break;
...
static int run_daemon(const char *cache_dir_arg) {
  ...
  /* Create a persistent window for selection conversion requests.
     Reused by get_selection() to avoid per-event
     XCreateSimpleWindow/XDestroyWindow round-trips. */
  ...
  if (!XFixesQueryExtension(dpy, &xfixes_event_base, &xfixes_error_base)) {
    ...
    if (daemon_win != None) { XDestroyWindow(dpy, daemon_win); daemon_win = None; }
    if (fd_primary >= 0) {
      close(fd_primary);
      fd_primary = -1;
    }
  ...
  /* Subscribe to PRIMARY owner-change events; content-change polling
     is handled by calling check_and_update_primary on each notification. */
  XFixesSelectSelectionInput(dpy, root, xa_primary,
                             XFixesSetSelectionOwnerNotifyMask);

  /* On WSL2, also subscribe to CLIPBOARD owner-change events.  WSLg bridges
     the Windows clipboard ↔ X11 CLIPBOARD; monitoring it lets us detect
     mouse selections made in Windows Terminal (which sets the Windows
     clipboard via copyOnSelect, NOT X11 PRIMARY). */
  if (monitor_clipboard) {
    XFixesSelectSelectionInput(dpy, root, xa_clipboard,
                               XFixesSetSelectionOwnerNotifyMask);
  }
  XFlush(dpy);
      ...
      while (XPending(dpy) > 0) {
        ...
        if (ev.type == xfixes_event_base + XFixesSelectionNotify) {
          ...
          if (sev->selection == xa_primary) {
            check_and_update_primary();
          } else if (monitor_clipboard && sev->selection == xa_clipboard) {
            check_and_update_clipboard();
          }
        }
      }
    }
 ...
 *   --copy-clipboard   Read stdin, take clipboard ownership, serve paste
 * requests.
 *   --clear-primary    Clear PRIMARY by setting its owner to None.
 * Modifier: --monitor-clipboard also monitors CLIPBOARD owner changes in
 * daemon mode (WSL2).
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 1; i < argc; i++) {
    if (strcmp(argv[i], "--oneshot") == 0)
      ...
      clear_primary = true;
    else if (strcmp(argv[i], "--monitor-clipboard") == 0)
      ...
      fprintf(
          ...
          "monitor-clipboard]\n"
          "XWayland selection monitor and clipboard helper for "
          "zsh-edit-select.\n\n"
          ...
          "  --monitor-clipboard    Also monitor CLIPBOARD changes (WSL2)\n",
          argv[0]);
      return 0;
    } else {
  ...
  /* Intern the standard X11 selection atoms for conversion requests.
     One batched request instead of four separate blocking round trips —
     XInternAtom is synchronous, so each call flushes and waits for its own
     reply.  XInternAtoms pipelines all four names and waits once, which
     matters most for the short-lived modes the shell forks per clipboard
     operation.  The array must be char *[] (not const): Xlib's prototype
     takes char **. */
  root = DefaultRootWindow(dpy);
  {
    char *atom_names[] = {"PRIMARY", "CLIPBOARD", "UTF8_STRING", "TARGETS"};
    Atom interned[4];
    XInternAtoms(dpy, atom_names, 4, False, interned);
