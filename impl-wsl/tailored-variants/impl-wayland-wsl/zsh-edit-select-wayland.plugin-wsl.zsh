...
# Establish the edit-select keymap and all related bindings inside an anonymous
    ...
    # Plain (unshifted) cursor-movement keys: resume WSL mouse tracking
    # when the user navigates with the keyboard after free-selection suspend.
    function _zes_move_and_resume_tracking() {
        _zes_resume_tracking_if_needed
        zle "${WIDGET#edit-select-move::}" -w
    }

    local -a move_bind=(
        'kcub1' '^[[D' 'backward-char'
        'kcuf1' '^[[C' 'forward-char'
        'khome' '^[[H' 'beginning-of-line'
        'kend'  '^[[F' 'end-of-line'
        ''      '^[[1;5D' 'backward-word'
        ''      '^[[1;5C' 'forward-word'
    )
    ...
    for ((mi = 1; mi <= ${#move_bind}; mi += 3)); do
        ...
        zle -N "edit-select-move::${mwid}" _zes_move_and_resume_tracking
        bindkey -M emacs "$mseq" "edit-select-move::${mwid}"
    done
