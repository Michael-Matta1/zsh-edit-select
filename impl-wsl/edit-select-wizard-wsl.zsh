...
function edit-select::apply-keybindings() {
	...
	# Bind the builtin undo/redo widgets directly.  The tailored variant —
	# the implementation actually loaded on WSL — never defines the
	# edit-select::undo / edit-select::redo wrapper widgets (only the
	# never-loaded fallback dispatcher does), so binding those names here
	# left wizard-applied undo/redo keys dead until the next shell start.
...
function edit-select::configure-undo() {
	...
	case "$choice" in
		1)
			...
			[[ -n "$_zes_old_key" ]] && { bindkey -M emacs -r "$_zes_old_key" 2>/dev/null; bindkey -r "$_zes_old_key" 2>/dev/null; bindkey -M edit-select -r "$_zes_old_key" 2>/dev/null; }
...
function edit-select::reset-keybindings() {
	...
	if [[ $confirm =~ ^[Yy]$ ]]; then
		...
		for _k in "$EDIT_SELECT_KEY_UNDO" "$EDIT_SELECT_KEY_REDO"; do
			[[ -n "$_k" ]] && { bindkey -M emacs -r "$_k" 2>/dev/null; bindkey -r "$_k" 2>/dev/null; bindkey -M edit-select -r "$_k" 2>/dev/null; }
		done
