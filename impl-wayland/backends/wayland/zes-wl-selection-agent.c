...
/* xdg-shell stable protocol — generated by wayland-scanner from the
   system xdg-shell.xml (see the XDG_SHELL_* rules in the Makefile).
   Needed for the 1x1 daemon surface Mutter requires to deliver PRIMARY
   selection events to background clients. */
...
/* Data-control source for --copy-clipboard (opaque wlr/ext pointer, cast via
   dc_use_ext).  Tracked so the cancelled handler can NULL it and the child's
   exit path can destroy it on the SIGTERM path. */
static void *dc_copy_source = NULL;
...
/* Set by the SIGUSR1 handler; consumed once per iteration of the daemon loop.
   sig_atomic_t + volatile is the only type the C standard lets a handler
   touch safely, which is why this is a flag rather than the work itself. */
static volatile sig_atomic_t drop_selection_req = 0;

/* SIGUSR1 handler — "the user dismissed the on-screen selection".
 *
 * Raised by the terminal click helper (assets/helpers/zes-alacritty-click-clear.sh)
 * against the pid in agent.pid.  It deliberately does NOT touch the compositor:
 * on KDE Plasma the clipboard manager treats an emptied PRIMARY as something to
 * restore and immediately refills it from its own history, so a client-side
 * clear comes back as a *new* selection carrying unrelated text (measured on
 * Plasma 6/Wayland: a stale `file:///…` URI replacing the real selection within
 * ~0.8 s, twice out of two attempts).  The shell then treats that text as the
 * active mouse selection and applies edits to it.
 *
 * Publishing the empty state straight from the daemon avoids the round trip
 * entirely: nothing is asked of the compositor, so there is nothing for a
 * clipboard manager to react to.  The compositor's own PRIMARY is left alone
 * and stays middle-click pasteable, which also means a selection made in
 * another application survives a click in the terminal.
 *
 * Async-signal-safety: this only assigns to a volatile sig_atomic_t.  All the
 * real work (cache write, seq bump) happens in the daemon loop below. */
static void drop_selection_handler(int sig) {
    (void)sig;
    drop_selection_req = 1;
}
...
static int ensure_cache_dir(const char *dir) {
    ...
    if (dir && dir[0]) {
        ...
        if (runtime) {
            ...
            if (!home) return -1;
            n = snprintf(cache_dir, sizeof(cache_dir), CACHE_DIR_TEMPLATE, home);
        }
    }
...
static void write_primary(const char *data, size_t len, unsigned long seq) {
    ...
    /* Fallback path: used for the pre-daemon() initial write (fd_primary is
       still -1 at that point), if open() failed after daemon(), and by
       --oneshot when it is given an explicit cache directory.  The other
       short-lived modes do not write the primary/seq cache. */
...
static char *read_fd_with_timeout(int fd, size_t *out_len, size_t max_size,
    ...
    while (1) {
        ...
        if (pfd.revents & POLLIN) {
            ...
            size_t avail = capacity - total;
            size_t want = avail > 65536 ? 65536 : avail;
...
static bool osc52_write(const char *data, size_t len) {
    ...
    /* Inline base64 encode into buf+7 */
    {
        ...
        size_t i = 0, j = 7;
        while (i < len) {
            /* The loop condition guarantees i<len for `a`; the second
               ternary is the structural complement for `b`/`c`, which may
               reach the end after `a` consumed the last byte. */
            uint32_t a = src[i++];
 ...
 * accepting any of them ensures compatibility with terminals, browsers,
 ...
 * Versions are capped at the highest interface level this agent was
 * written against; every request it issues on these objects is available
 * since version 1 of the respective interface.
...
static void registry_handle_global(void *data, struct wl_registry *reg,
    ...
    if (strcmp(interface, "wl_seat") == 0) {
        /* Cap at the advertised version, matching wl_data_device_manager,
           wl_compositor and zwlr_data_control_manager_v1 below.  This agent
           only issues get_keyboard/destroy, both available since version 1,
           so requesting a version the compositor does not advertise would be
           a protocol error for no gain.  Every current compositor advertises
           wl_seat >= 5, so this evaluates to 2 exactly as before. */
        wl_seat_obj = wl_registry_bind(reg, name, &wl_seat_interface,
                                        version < 2 ? version : 2);
...
/* ===== MODE: --oneshot ============================================ */
/* On Mutter/GNOME, PRIMARY selection events are delivered to clients with
   a mapped surface.  We briefly create a tiny popup surface (with an empty
   input region so it cannot steal keyboard focus), let Mutter deliver the
   current selection, read it, and exit.  The surface is transparent and
   exists for only a few milliseconds. */
...
static int run_oneshot(const char *cache_dir_arg) {
    ...
    /* If we didn't get the selection event (Mutter), map the daemon
       surface so the compositor delivers the PRIMARY offer to it, then
       dispatch events until the selection arrives or we time out.  The
       daemon surface has an empty input region and cannot steal keyboard
       focus; it is mapped only so Mutter routes the offer. */
    if (!got_selection && wl_comp && xdg_wmbase && wl_shm_obj) {
        /* create_daemon_surface returns -1 only before any surface is
           committed/mapped (shm/buffer/surface allocation failure).  With no
           mapped surface the compositor will never deliver a PRIMARY event, so
           waiting would just stall the oneshot read for ~5 s with no chance of
           success — skip the wait and fall through to the cache-write below,
           matching run_daemon's own failure handling. */
        if (create_daemon_surface() == 0) {
            ...
            while (!got_selection && attempts < 50) {
                ...
                /* Break on unrecoverable display errors, mirroring the four
                   other poll loops in this file (run_get_clipboard,
                   run_copy_clipboard, run_daemon).  With a dead display no
                   selection event can ever arrive, so the output is identical
                   to timing out — only the up-to ~5 s stall is avoided.
                   EINTR is a signal interruption, not an error: keep retrying. */
...
static int run_copy_clipboard(void) {
    ...
    /* Tracked at function scope so the child exit path can destroy them;
       run_get_clipboard already destroys its data-control devices the same way. */
    struct ext_data_control_device_v1 *ext_dc_dev = NULL;
    struct zwlr_data_control_device_v1 *wlr_dc_dev = NULL;
    ...
    if (ext_dcm || wlr_dcm) {
        if (ext_dcm) {
            ...
            /* The device receives data_offer/selection announcements while
               the child serves; without a listener each announced offer leaks
               an anonymous proxy.  The shared listeners track offers in the
               dc_clipboard_offer/dc_primary_offer globals, destroyed at exit. */
            ext_data_control_device_v1_add_listener(ext_dc_dev,
                &dc_device_listener_ext, NULL);
            ...
            ext_data_control_device_v1_set_selection(ext_dc_dev, dc_src);
            dc_copy_source = dc_src;
            wl_display_flush(wl_dpy);
            dc_copy_done = true;
        } else {
            dc_use_ext = false;
            ...
            zwlr_data_control_device_v1_set_selection(wlr_dc_dev, dc_src);
            dc_copy_source = dc_src;
            wl_display_flush(wl_dpy);
            dc_copy_done = true;
        }
    ...
    pid_t pid = fork();
    if (pid < 0) {
        /* fork failed — destroy the data-control objects we created above
           before disconnecting.  wayland_disconnect() does not own them, so
           without this they leak until process exit.  Mirrors the child's
           exit path; the copy still fails with return 1. */
        if (dc_copy_source) {
            if (dc_use_ext)
                ext_data_control_source_v1_destroy(dc_copy_source);
            else
                zwlr_data_control_source_v1_destroy(dc_copy_source);
            dc_copy_source = NULL;
        }
        if (ext_dc_dev) ext_data_control_device_v1_destroy(ext_dc_dev);
        if (wlr_dc_dev) zwlr_data_control_device_v1_destroy(wlr_dc_dev);
    ...
    if (pid > 0) {
        /* Parent returns immediately so the shell is never blocked.  Free our
           own post-fork copy of the clipboard data so leak checkers report the
           fork-server pattern clean; the child owns the Wayland connection and
           its own copy, which it frees on exit.  Do NOT cleanup wayland here. */
        free(copy_data);
        _exit(0);
    }
...
static int run_daemon(const char *cache_dir_arg) {
    ...
    /* Selection-dismissed notification from the terminal click helper.
       Registered only for the daemon: it is the process that owns the cache
       files, so it is the only one that may publish to them. */
    signal(SIGUSR1, drop_selection_handler);
    ...
    /* Open persistent fds for write_primary() hot path.
       ...
       O_CREAT without O_TRUNC: the first real write overwrites in place
       via pwrite at offset 0 + ftruncate (see write_primary's hot path). */
    ...
    if (ext_dcm) {
        ...
        ext_dc_dev = ext_data_control_manager_v1_get_data_device(ext_dcm, wl_seat_obj);
        ext_data_control_device_v1_add_listener(ext_dc_dev, &dc_device_listener_ext, NULL);
    } else if (wlr_dcm) {
        dc_use_ext = false;
        dc_primary_offer = NULL;
        dc_primary_has_text_sel = false;
        wlr_dc_dev = zwlr_data_control_manager_v1_get_data_device(wlr_dcm, wl_seat_obj);
        ...
        if (create_daemon_surface() != 0) {
            ...
            free(last_known_content);   /* may be NULL — free(NULL) is safe  */
            last_known_content = NULL;
     ...
     * With data-control (wlroots/KDE/GNOME 47+): purely event-driven — the
     * compositor sends a selection event when PRIMARY changes →
     * dc_device_primary_selection_* fires → reads content and updates the
     * cache immediately.  The Wayland protocol spec says PRIMARY selection
     * clients should call set_selection only once after the operation
     * finishes (mouse release); GTK3/GTK4 terminals comply, firing on
     * button-release with the final content, so the cache is correct before
     * the user invokes Ctrl+C.  The loop polls every 1 s here only for
     * SIGTERM processing.
     * Without data-control (GNOME < 47, ps_manager only): Mutter does not
     * deliver PRIMARY selection events to an unfocused background surface,
     * so the loop polls every 50 ms and re-reads the current offer via
     * process_primary_update (the if(changed) dedup still gates cache
     * writes — see the ret==0 branch below).  The 1x1 daemon surface above
     * is mapped so Mutter delivers the initial offer; GNOME/Mutter normally
     * routes through the xwayland agent instead (see wayland-backend.zsh),
     * so this path is a fallback for Mutter-like DEs without XWayland. */
    int wl_fd = wl_display_get_fd(wl_dpy);

    while (running) {
        /* Consume a pending SIGUSR1 before blocking again.  The handler can
           fire at any point, including while poll() is asleep — poll() then
           returns EINTR and the `continue` below brings us straight back
           here.  Publishing an empty PRIMARY is exactly what
           process_primary_update() does when a selection genuinely goes away,
           so the shell sees the (new seq, empty primary) pair it already
           knows how to handle, and last_known_content is reset so re-selecting
           the very same text afterwards still registers as a change. */
        if (drop_selection_req) {
            drop_selection_req = 0;
            seq_counter++;
            write_primary("", 0, seq_counter);
            free(last_known_content);
            last_known_content = NULL;
