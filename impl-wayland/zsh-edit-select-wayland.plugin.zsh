...
# next change comparison has a current baseline; without this re-read
...
function _zes_sync_after_paste() {
    ...
    if ((_EDIT_SELECT_DAEMON_ACTIVE)); then
        ...
        # group suppresses an inherited err_return abort only while the caller
        # ignores the enclosing function's status, and the cut/copy/paste
        # widgets that call this are themselves invoked through a status-
        # observing wrapper when zsh-syntax-highlighting is loaded.  Both reads
        # are best-effort re-baselining; a daemon unlink race must not abort
        # before the widget finishes its own kill-region / paste.
...
function _zes_resolve_multiline() {
    ...
    local row1_content
    for (( i = 1; i <= n - m + 1; i++ )); do
        # Continuation rows must match exactly, which is a handful of string
        # compares; the row-1 suffix search below walks every prefix length with
        # a glob each.  Test alignment first and skip the search for offsets that
        # cannot match anyway.  Both tests are pure and row1_content is read only
        # after both pass, so the order does not change the result.
        aligned=1
        for (( k = 2; k <= m; k++ )); do
            [[ "${_sl[k]}" != "${_bl[i + k - 1]}" ]] && { aligned=0; break; }
        done
    ...
    for (( i = 1; i <= n - m + 1; i++ )); do
        # Cheap tests first — interior-row alignment, then the last-row prefix
        # requirement — before resolving row 1, whose fallback walks every
        # prefix length with a glob each.  All three are pure and row1_content
        # is read only once every test has passed, so they commute.
        aligned=1
        for (( k = 2; k <= m - 1; k++ )); do
            [[ "${_sl[k]}" != "${_bl[i + k - 1]}" ]] && { aligned=0; break; }
        done
...
function _zes_refine_trim_candidate() {
    ...
    while [[ "$buf" == *"$cand"* ]]; do
        ...
        idx=$((idx + ${#prefix} + clen))
        # || true: a bare `(( cnt++ ))` evaluates to 0 on the first iteration
        # (post-increment yields the OLD value), which is a nonzero exit status.
        # Under a user's inherited err_return/err_exit that aborts this function
        # mid-count, so the caller sees an empty REPLY and the selection is
        # silently dropped.  The status is unobserved; cnt is unchanged.
        (( cnt++ )) || true
    ...
    if (( cnt == 1 )); then
        ...
        if [[ -n "$stripped" && slen -lt clen ]]; then
            ...
            while [[ "$buf" == *"$stripped"* ]]; do
                prefix="${buf%%"$stripped"*}"
                idx=$((idx + ${#prefix} + slen))
                # || true: same first-iteration zero-value abort as above.
                (( cnt2++ )) || true
...
function _zes_detect_mouse_selection() {
    ...
    if ((_EDIT_SELECT_DAEMON_ACTIVE)); then
        if ((_EDIT_SELECT_NEW_SELECTION_EVENT)); then
            ...
            # is_new_selection=1, so mouse_sel is unused here.  Test the cached
            # primary for emptiness via ${var:+x}, which yields a one-byte
            # result regardless of selection size — `[[ -z "$var" ]]` would
            # materialise the whole value on every no-event keystroke (the
            # hottest path), which is O(selection-size) for a large scrollback
            # drag that the matcher could not resolve.
...
function _zes_terminal_focus_in() {
    if ((_EDIT_SELECT_DAEMON_ACTIVE)); then
        local current_seq
        # `|| true` inside the brace group: the read status is unobserved (the
        # emptiness test below is the real gate), but a daemon that unlinked seq
        # between the DAEMON_ACTIVE test and this read would otherwise abort the
        # widget under an inherited err_return — skipping the two state clears
        # below, which are the whole point of this handler.  The guard must sit
        # INSIDE the braces: a trailing `|| true` on a brace group only
        # suppresses the abort while the caller ignores this function's status,
        # and zsh-syntax-highlighting's widget wrapper does observe it.
...
function edit-select::zle-line-pre-redraw() {
    ...
    if ((_EDIT_SELECT_DAEMON_ACTIVE)); then
        ...
        if [[ "$current_seq" != "$_EDIT_SELECT_LAST_SEQ" ]]; then
            # New seq: agent wrote a selection change.  Read and signal it.
            _EDIT_SELECT_LAST_SEQ="$current_seq"
            { local new_primary=$(<"$_EDIT_SELECT_PRIMARY_FILE") } 2>/dev/null
            _EDIT_SELECT_LAST_PRIMARY="$new_primary"
            if [[ -n "$new_primary" ]]; then
...
function edit-select() {
    case "$1" in
    conf | config)
        local wizard_file="$_EDIT_SELECT_PLUGIN_DIR/edit-select-wizard-wayland.zsh"
...
source "$_EDIT_SELECT_PLUGIN_DIR/backends/wayland-backend.zsh"
...
# Pre-populate LAST_SEQ before the first ZLE callback fires so the initial
# redraw does not see a spurious empty-to-non-empty transition.
if ((EDIT_SELECT_MOUSE_REPLACEMENT)); then
    _zes_start_monitor || true
